...
app.use((err: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err instanceof ResourceNotFoundError) {
    statusCode = 404;
  } else if (err instanceof UserAlreadyExistsError || err instanceof InvalidCredentialsError) {
    statusCode = 400;
  } else if (err instanceof UserAlreadySubscribedError || err instanceof RegistrationPeriodEndedError || err instanceof ParticipantsLimitReachedError || err instanceof PedalNameAlreadyExistsError) {
    statusCode = 409;
  }
